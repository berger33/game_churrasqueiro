...
then its follow-ups: see "FTUE follow-ups" below; then the AI 2D art pass, lote 01 — see
...
## AI 2D art pass — lote 01 (docs/22-ARTE_2D_PLANO.md)
...
- **Lote 01: 10 images → 43 sprites**:
  - 5 foods × (raw, rare, medium, well, burned + served);
  - the lata grill with a transparent cooking opening;
  - the Quintal backdrop;
  - embers at 3 heats;
  - the counter;
  - 7 customer portraits.
- **Status: pending approval.** Every registry row is `status=pending`; no game code uses the
  sprites yet.
- **Tools** (both deterministic, about 3 s per batch):
  - `tools/art/process-sprites.mjs` keys out the flat magenta background per image,
    un-mixes soft edges, erases the grid lines the model draws anyway, drops captions it
    wrote, aligns the 5 states of a food onto one canvas and pivot, and finds the grill's hole
    quad. It writes the PNG masters to `Assets/Art`, plus `sprites.manifest.json` and
    `ASSET_REGISTRY.csv`.
  - `tools/art/review-sheet.mjs` renders the contact sheet and a montage in the game's
    420×780 layout. The montage is labelled "not a game capture" and is never a store
    screenshot.
- **Verified:** 0 residual magenta pixels across the 43 sprites; edges checked at 1:1 on light
  and grey backgrounds.
- ⚠ **Not verified:** the sprites are not yet integrated into the game (docs/22 §7), and the
  `check-art-registry` gate is planned, not written. The gates are unchanged (14).
- Raw model outputs (17 MB) stay out of git (`art/source/`); the masters are 9.1 MB.
...
7. **Professional 2D art** (docs/22):
   - get lote 01 approved (open decisions: keep the oblique lata? redo the shirtless turista?);
