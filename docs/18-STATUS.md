...
then its follow-ups: see "FTUE follow-ups" below; then the AI 2D art pass, lotes 01–02 — see
...
## AI 2D art pass — lotes 01–02 (docs/22-ARTE_2D_PLANO.md)

The mechanics tested well, but the procedural canvas art still read as a prototype. The plan
(docs/22) replaces it with painted sprites generated by an image model in batches of
10 images. Each batch is reviewed and **approved by the game's owner before the next one is
generated**.

- **Lote 01: 10 images → 43 sprites. Approved 2026-09-25.** It contains:
  - 5 foods × (raw, rare, medium, well, burned + served);
  - the lata grill (evo 1) with a transparent cooking opening;
  - the Quintal backdrop, embers at 3 heats and the counter;
  - 7 customer portraits.

  Decisions recorded in the registry: the oblique lata stays (1 zone, 2–3 slots); the shirtless
  turista stays for now, and shirted versions came in lote 02.
- **Lote 02: 10 images → 50 sprites. Pending approval.** It contains:
  - 6 grill foods (espetinho de frango, coração, coxa, asinha, fraldinha, legumes);
  - the vinagrete as a 4-frame prep item;
  - 8 customer portraits, which cover the 5 missing types plus variants;
  - lata evo 2 and evo 3.

  The evolutions kept evo 1's camera: the opening tilts −8.6°, −8.7° and −8.8°.
- **Tools** (deterministic, about 3 s per batch):
  - `tools/art/process-sprites.mjs` keys out the flat magenta background, un-mixes soft edges,
    erases stray grid lines, drops captions, aligns the states of a food onto one canvas and
    pivot, and finds the grill's hole quad. It writes the masters to `Assets/Art`, plus
    `sprites.manifest.json` and `ASSET_REGISTRY.csv`. A missing raw file is skipped and the
    committed masters are kept.
  - `tools/art/review-sheet.mjs` renders the contact sheet and a montage in the 420×780 layout,
    labelled "not a game capture" (never a store screenshot).
  - `tools/art/set-status.mjs` records the owner's decision in the registry.
  - `tools/art/make-ref.mjs` rebuilds style references from approved masters.
- **Verified:** 0 residual magenta pixels across the 89 cut-out sprites; edges checked at 1:1
  on light and grey backgrounds; re-running lote 01 without its raw files changes nothing.
- ⚠ **Not verified / not built:**
  - the sprites are not yet integrated into the game (docs/22 §7);
  - `check-art-registry` is planned, not written;
  - the gates are unchanged (14).
- **Storage:**
  - Raw model outputs are disposable and gitignored. A workspace reset wiped lote 01's raw
    files, so masters are now processed and committed in the same turn as generation.
  - Masters: 9.4 MB (lote 01) and 6.7 MB (lote 02).
...
7. **Professional 2D art** (docs/22):
   - ~~get lote 01 approved~~ — **approved**;
   - get lote 02 approved (open decisions: which turista stays? redo the pale "rare" heart?);
   - integrate the approved sprites into the prototype, keeping the procedural fallback (docs/22 §7.1);
