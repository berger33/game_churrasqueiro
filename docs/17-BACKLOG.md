...
| 5 | Tutorial (6 steps) in Unity, non-skippable first run | M | A new player completes a perfect serve with zero text explanation |
...
| 11 | Analytics SDK integration (Firebase) with the 47-event taxonomy | M | Every event in `analytics.json` fires in a debug build and appears in DebugView |
