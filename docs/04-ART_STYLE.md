...
1. `tools/art/process-sprites.mjs` writes each new row with `source=ai-assisted` and
   `status=pending`.
2. When the batch is approved, the row becomes `source=ai-assisted-reviewed` and
   `status=approved`. A refused asset becomes `status=rejected`, and its redo leads the next batch.
3. Only `approved` rows may ship. Re-running the processor never resets a status.
...
registry of §11 exists since the AI art pass (lote 01: 43 sprites, `status=pending`), but its
CI check does not yet. The prototype's coals, glow and gradients are canvas
